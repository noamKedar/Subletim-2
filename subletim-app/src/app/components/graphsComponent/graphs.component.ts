...
export class GraphsComponent {
  ...
  secondGraphData = {};
  secondGraphDataArray: { numOfRooms: number, amountOfApartments: number }[] = [];
...
// add title to the graph
    ...
    this.apartmentService.mapReduce().subscribe(data => {
      ...
      // Y axis
      var y2 = d3.scaleBand()
        .domain(this.secondGraphDataArray.map(function (d) {
          return d.numOfRooms;
        }))
        .range([0, height])
        .padding(1);
      ...
      svg2.selectAll("myline")
        ...
        .append("line")
        .attr("x1", function (d) {
          return x2(d.amountOfApartments);
        })
        .attr("x2", x2(0))
        .attr("y1", function (d) {
      ...
      svg2.append("text")
        ...
        .style("font-size", "16px")
        .style("text-decoration", "underline")
        .text("Amount Of Apartments By Number Of Rooms");
    });
  }
}
